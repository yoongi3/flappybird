...
const BOSS = new Audio();
BOSS.src = "audio/song_boss.wav"
BOSS.volume = 0.4;

const LIVE = new Audio();
LIVE.src = "audio/song_alive.wav"
LIVE.volume = 0.4;

const CURB = new Audio();
CURB.src = "audio/song_curb.wav"
CURB.volume = 0.6;

const SAD = new Audio();
SAD.src = "audio/song_sad.wav"
SAD.volume = 0.6;

const GAE = new Audio();
GAE.src = "audio/gae.wav"
GAE.volume = 0.4;
...
const gameOver = {
	...
	draw : function(){
		if(state.current == state.over){
			ctx.drawImage(sprite, this.sX, this.sY, this.w, this.h, this.x, this.y,
			this.w, this.h);
			if(score.value == score.best - 1 || score. value == score.best){
				CURB.play();
				BOSS.pause();
				BOSS.currentTime=0;
				LIVE.pause();
	...
	update: function(){
		...
		for(let i = 0; i < this.position.length; i++){
			...
			&& bird.y + bird.radius > p.y && bird.y - bird.radius < p.y +this.h){
				state.current = state.over;
				if (score.value > 5 && score.value < score.best - 10){
					GAE.play();
				}
			}
			//BOTTOM PIPE
			...
			}if (score.value >= 10 && score.value >= score.best - 10){
				BOSS.play();
				LIVE.pause();
				LIVE.currentTime=0;
			}else{
